...
public:
    ...
    void movement(int x, int y, int z, int c);
    void insert_character(char& x);
    bool check_end();
    bool check_bottom();
    void home_end(int x);
    void new_line();
    void redo_newline(int x, int y);
    void backspace();
    void delete_line();
    char returnchar(int x, int y);
    std::string returnline(int x);
    void redo_deleteline(std::string x, int y,int z);
    void undo_backspace(char& x);

private:
    // Write declarations for any private member variables here.
    // Additionally, you can add any private member functions you'd like.
    unsigned int row, column, size;
    std::string eMessage;
